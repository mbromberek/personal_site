...
from app.main.forms import EmptyForm, WorkoutCreateBtnForm, WorkoutForm, WorkoutFilterForm, WorkoutIntervalForm, WorkoutExportForm, UserSettingsForm, GearForm, LocForm, TagForm, GoalForm
...
def edit_goal():
    ...
    logger.debug('wrkt_type_select_lst: {}'.format(wrkt_type_select_lst))
    ...
    if request.method == 'GET':
        ...
        logger.debug('goal_form.workout_type_lst.data: {}'.format(goal_form.workout_type_lst.data))
        if goal_form.workout_type_lst.data == -1:
            goal.workout_type_id = None
        else:
            goal.workout_type_id = goal_form.workout_type_lst.data
        logger.debug('goal.workout_type_id: {}'.format(goal.workout_type_id))
    ...
    # logger.debug(wrkt_type_dict)
    # logger.debug(goal.workout_type_id)
    default_type = None
    for key, value in wrkt_type_dict.items():
        ...
        if value == goal.workout_type_id:
            default_type = value
            break
    # logger.debug(default_type)
    goal_form.workout_type_lst.default = default_type if default_type != None else None
    logger.debug('goal.workout_type_id: {}'.format(goal.workout_type_id))
    logger.debug('workout_type_lst default {}'.format(default_type))
